...
parser.add_argument('--yolo_model', help='Path to YOLOv2 h5 model', required=False)
...
def EDdat(loc, gt):
    groundtruth = np.unravel_index(gt.argmax(), gt.shape)
    dist = np.linalg.norm(np.asarray([loc[1], loc[0]]) - np.asarray(groundtruth))
    print(dist)
    # plt.imshow(gt)
    # plt.scatter(x=loc[0], y=loc[1], c='r', s=60)
    # plt.scatter(x=groundtruth[1], y=groundtruth[0], c='r', s=60)
    # plt.show()

    return dist, loc[0], loc[1], groundtruth[0], groundtruth[1]

# Get box for specific class 
def get_box_for_class(boxes, class_name):
  for box in boxes:
...
for i in ["_joint_test.csv"]:
    if ".dat" in args.path_trained_model:
        ...
        model.load_weights(args.path_trained_model)
    elif "yolov2" in args.path_trained_model:
        model = YoloV2(input_shape=(256, 256, 3),
                   GRID_H=8, GRID_W=8) 
        model.load_weights('weights/yolov2_50.819252.h5')
        ...
        try:
            ...
            if type(img) != list:
                if ".dat" in args.path_trained_model:
                    ...
                    foveaLoc = (output.part(1).y, output.part(1).x)
                    diskLoc = (output.part(0).y, output.part(0).x)
                    # plt.imshow(img)
                    # plt.scatter(x=[foveaLoc[0]], y=[foveaLoc[1]], c='r', s=60)
                    # plt.scatter(x=[diskLoc[0]], y=[diskLoc[1]], c='r', s=40)

                    # plt.show()
                    EDFovea, fpX, fpY, fgX, fgY = EDdat(foveaLoc, Fovea)
                    EDDisk, dpX, dpY, dgX, dgY = EDdat(diskLoc, Disk)
                elif "yolov2" in args.path_trained_model:
                    img_path = os.path.join(os.path.split(args.dd)[0], r[1], r[0]) 
                    image = Image.open(img_path)
                    image = resize_image(image, args.img, args.img)
                    
                    boxes = yolo.predict(image)
                    fovea_box = get_box_for_class(boxes, 'Fovea')
                    disk_box = get_box_for_class(boxes, 'Optic Disc')
                    
                    fpX, fpY, fgX, fgY = get_center(fovea_box)
                    dpX, dpY, dgX, dgY = get_center(disk_box)
                    
                    EDFovea, _, _, _, _ = EDdat((fpX, fpY), Fovea)
                    EDDisk, _, _, _, _ = EDdat((dpX, dpY), Disk)
                else:
