...
#     pool_2 = layers.MaxPooling2D(pool_size=(2, 2))(dn_2)
    ...
    #
    # conv_final = layers.Conv2D(2, kernel_size=(1, 1))(up_conv_2)
...
def attentionunet(input_shape, dropout=0.2, batchnorm=True):
    ...
    dn_5 = conv_block(pool_4, kernelsize, filters[4], dropout, batchnorm)
    pool_5 = layers.MaxPooling2D(pool_size=(2, 2))(dn_5)

    dn_6 = conv_block(pool_5, kernelsize, filters[5], dropout, batchnorm)


    # Upsampling layers
    gating_6 = gatingsignal(dn_6, filters[4], batchnorm)
    att_6 = attention_block(dn_5, gating_6, filters[4])
    up_6 = layers.UpSampling2D(size=(upsample_size, upsample_size), data_format="channels_last")(dn_6)
    up_6 = layers.concatenate([up_6, att_6], axis=3)
    up_conv_6 = conv_block(up_6, kernelsize, filters[4], dropout, batchnorm)

    gating_5 = gatingsignal(up_conv_6, filters[3], batchnorm)
    att_5 = attention_block(dn_4, gating_5, filters[3])
    up_5 = layers.UpSampling2D(size=(upsample_size, upsample_size), data_format="channels_last")(up_conv_6)
    up_5 = layers.concatenate([up_5, att_5], axis=3)
    up_conv_5 = conv_block(up_5, kernelsize, filters[3], dropout, batchnorm)
